...
public class PodNodeScheduler {
    protected static PodListWithIndex unscheduledPods;

    private PodNodeScheduler() {

    }

    static synchronized Optional<Pair<Pod, Node>> schedule(Pod podToSchedule, PodListWithIndex existingPods, NodeList nodeList) {
        //   System.out.println("pod to sched " + (podToSchedule != null ? podToSchedule.getMetadata().getName() :
        //          " none ") + " existing size " + existingPods.getItems().size() + " unassigned size " + (unassignedPods != null ? unassignedPods.getItems().size() : "no unassigned") + "; nodes " + nodeList.getItems().size())
        //  ;
        if (podToSchedule != null) {
            return scheduleSingle(podToSchedule, existingPods, nodeList);
        } else {
            scheduleQueue(existingPods, nodeList);
            return Optional.empty();
        }
    }


    static Optional<Pair<Pod, Node>> scheduleSingle(Pod podToSchedule, PodList existingPods, NodeList existingNodes) {
        ...
        if (nodeScore.isEmpty()) {
            ...
            PodNodeScheduler.unscheduledPods.addPodToList(podToSchedule);
            return Optional.empty();
        } else {
            ...
            PodNodeScheduler.unscheduledPods.getItems().forEach(podWithAge -> ((PodWithAge) podWithAge).setAge(((PodWithAge) podWithAge).getAge().add(BigDecimal.ONE)));
            System.out.println("Pod " + podToSchedule.getMetadata().getName() + " will be bound to node.");
    ...
    static void scheduleQueue(PodListWithIndex existingPods, NodeList existingNodes) {
        if (PodNodeScheduler.unscheduledPods.getItems().isEmpty()) {
            return;
        }
        Optional<Pod> starvingPod = getStarvingPod();
        if (starvingPod.isPresent()) {
            scheduleSingle(starvingPod.get(), existingPods, existingNodes);
        } else {
            List<NodeWithAlloc> nodesWithAllocList = estimateFreeNodeCapabilities(existingPods, existingNodes);

            AtomicReference<Triplet<Pod, Node, Integer>> nodePodWithHighestScore = new AtomicReference<>(new Triplet<>(null, null, -1));

            PodNodeScheduler.unscheduledPods.getItems().forEach(pod -> {
            ...
            if (nodePodWithHighestScore.get().getValue2() > -1) {
                ...
                PodNodeScheduler.unscheduledPods.getItems().remove(nodePodWithHighestScore.get().getValue0());
                //raise everyone else's age
                PodNodeScheduler.unscheduledPods.getItems().forEach(podWithAge -> ((PodWithAge) podWithAge).setAge(((PodWithAge) podWithAge).getAge().add(BigDecimal.ONE)));

                Pod podToAdd = pair.getValue0();
                podToAdd.getSpec().setNodeName(pair.getValue1().getMetadata().getName());
                existingPods.addPodToList(podToAdd);
            }
    ...
    public static Optional<Pod> getStarvingPod() {
        ...
        Pod oldestPod = PodNodeScheduler.unscheduledPods.getItems().get(0);
        BigDecimal averageAge = BigDecimal.ZERO;
        BigDecimal stdDev = BigDecimal.ZERO;
        for (Pod pod : PodNodeScheduler.unscheduledPods.getItems()) {
        ...
        averageAge = averageAge.divide(BigDecimal.valueOf(PodNodeScheduler.unscheduledPods.getItems().size()), RoundingMode.CEILING);
        for (Pod pod : PodNodeScheduler.unscheduledPods.getItems()) {
            PodWithAge pwa = (PodWithAge) pod;
        ...
        stdDev = stdDev.divide(BigDecimal.valueOf(PodNodeScheduler.unscheduledPods.getItems().size()).subtract(BigDecimal.ONE), RoundingMode.CEILING);
